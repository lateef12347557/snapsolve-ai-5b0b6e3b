...
const SolverPage = () => {
  ...
  const handleSolve = async () => {
    if (!question.trim() && !imagePreview) { toast.error("Please enter a question or upload an image"); return; }
    setIsLoading(true);
    setSolution("");
    ...
    <DashboardLayout>
      <div className="max-w-3xl mx-auto">
        ...
        <motion.div className="text-center mb-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-muted-foreground text-sm">Upload a photo or type your problem — get step-by-step AI explanations</p>
        </motion.div>

        {/* Subject selector */}
        <motion.div className="flex justify-center gap-3 mb-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
          {subjects.map((s) => (
            <button
              key={s.id}
